...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 23 23:31:26 2019
...
#df['CATEGORY'] = df['CATEGORY'].map({'D':4, 'C':3, 'B':2, 'A':1})
...
y_train = train['CATEGORY'].map({'D':3, 'C':2, 'B':1, 'A':0}).values
y_test = test['CATEGORY'].map({'D':3, 'C':2, 'B':1, 'A':0}).values
...
clf_lgb = lgb.LGBMClassifier(random_state=1337)
clf_lgb.fit(X_train, y_train)

# Predicting the Test set results
y_pred = clf_lgb.predict(X_train)
...
clf_lgb = lgb.LGBMClassifier(learning_rate=0.1, n_estimators=100, objective='multiclass', num_class = 4, verbosity = -1, random_state= 1337)

parameters = {
              'learning_rate':[0.1],
              'num_leaves':[15,20,25], 
              'min_sum_hessian_in_leaf':[0,0.1,0.5,1,10],
              'bagging_freq' : [3,5,7],
...
#
n_fold = 10
folds = StratifiedKFold(n_splits=n_fold, shuffle=True, random_state=1337)
lgb_params = {
    "objective" : 'multiclass',
    "num_class" : 4,
    "metric" : 'multi_error',
...
for n_fold, (train_idx, valid_idx) in enumerate(folds.split(X_train, y_train)):
        ...
        clf_lgb.fit(train_x, train_y, 
                eval_set = [(train_x, train_y), (valid_x, valid_y)], eval_metric = 'multi_error', 
                verbose = 100, early_stopping_rounds = 50)
        prediction += clf_lgb.predict(X_test)
        cvscores.append(accuracy_score(y_test, clf_lgb.predict(X_test)))
prediction /= n_fold
...
cvscores = []
for i in range(0, 10):
    indep_train, indep_test, dep_train, dep_test = train_test_split(X_train, y_train, test_size=0.3, random_state=i)
    clf_lgb = lgb.LGBMClassifier(**params)
    clf_lgb.fit(indep_train, dep_train, 
                eval_set=[(indep_train, dep_train), (indep_test, dep_test)], 
                eval_metric = 'multi_error', 
                verbose = 100,
                early_stopping_rounds = 50)
    model.append(clf_lgb)
    cvscores.append(accuracy_score(y_test, clf_lgb.predict(X_test)))
