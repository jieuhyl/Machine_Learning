# -*- coding: utf-8 -*-
"""
Created on Thu Aug 27 01:26:14 2020
...
from sklearn.feature_selection import GenericUnivariateSelect, f_classif, mutual_info_classif, chi2, f_regression, mutual_info_regression
gus = GenericUnivariateSelect(chi2, mode='k_best', param=4)
#gus = GenericUnivariateSelect(chi2, mode='percentile', param=30)
#X_reduced = gus.fit_transform(X, y)
gus.fit(X, y)

print(gus.get_support(indices=True))
#gus.scores_


pickup = df.columns[gus.get_support(indices=True)+2].tolist()
...
vt.fit(X, y)
#X_reduced = vt.fit_transform(X, y)

vt.variances_
print(vt.get_support(indices=True))


pickup = df.columns[vt.get_support(indices=True)+2].tolist()
